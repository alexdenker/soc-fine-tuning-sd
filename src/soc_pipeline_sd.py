...
class SOCStableDiffusionPipeline(
    StableDiffusionPipeline,
    DiffusionPipeline,
        ...
        **kwargs,
        ...
        Args:
            ...
            learn_offset (`bool`, *optional*, defaults to `False`):
                True if self.unet outputs the offset between the fine-tuned model and the pre-trained model, False if
        ...
        positive_prompt_embeds = prompt_embeds.clone()
            ...
            for i, t in enumerate(timesteps):
                ...
                if learn_offset:
                    noise_pred_init = self.unet_init(
                        ...
                        encoder_hidden_states=prompt_embeds.to(self.unet.dtype),
                    ...
                    # perform guidance
                    if self.do_classifier_free_guidance:
                        noise_pred_init_uncond, noise_pred_init_text = noise_pred_init.chunk(2)
                        noise_pred_init = noise_pred_init_uncond + self.guidance_scale * (
                            noise_pred_init_text - noise_pred_init_uncond
                        )
                        if not use_init_model:
                            noise_pred_offset_uncond, noise_pred_offset_text = noise_pred_offset.chunk(2)
                            noise_pred_offset = noise_pred_offset_uncond + self.guidance_scale * (
                                noise_pred_offset_text - noise_pred_offset_uncond
                            )
                    ...
                    if self.do_classifier_free_guidance and self.guidance_rescale > 0.0:
                        if use_init_model:
                            noise_pred_text = noise_pred_init_text
                        else:
                            noise_pred_text = noise_pred_init_text + noise_pred_offset_text
                    ...
                    noise_pred = unet_model(
                        latent_model_input.to(unet_model.dtype),
                        t.to(unet_model.dtype),
                        encoder_hidden_states=prompt_embeds.to(unet_model.dtype),
                        timestep_cond=timestep_cond,
                        cross_attention_kwargs=self.cross_attention_kwargs,
                        added_cond_kwargs=added_cond_kwargs,
                        return_dict=False,
...
def latent_to_decode(*, model, output_type, latents):
    if not output_type == "latent":
        ...
        has_latents_mean = (
            hasattr(vae.config, "latents_mean")
            and vae.config.latents_mean is not None
        )
        has_latents_std = (
            hasattr(vae.config, "latents_std")
        ...
        if has_latents_mean and has_latents_std:
            ...
            latents = (
                latents * latents_std / vae.config.scaling_factor + latents_mean
            )
